...
void TouchscreenDriver::ProcessPositionData(void)
{
	...
	if(sizecontact >= round(ADC_BUF_LEN*.75)) //If there are any points of contact
	{
		sizemidpt = round(sizecontact*(.5));
		...
		delT = ((float)currtime-(float)prevtime);
		...
		//**Should velocity be filtered more as well?**
		if(absxvel <= 800 && absyvel <= 800) //If velocity is below 100 mm/s (change depending on results)
		{
			n = 0;
			xpos = xposcurrent;
			ypos = yposcurrent;
...
////Change sort to merge sort for faster processing
