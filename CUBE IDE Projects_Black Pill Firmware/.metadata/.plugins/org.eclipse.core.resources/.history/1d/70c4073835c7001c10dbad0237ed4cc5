						   ...
						   float IMUy, float GyroY)
{
	...
	if (buffer[0] != '\0') //Double-Check if USB buffer has received message from Host PC
	{
		...
		GlobalVars::m0tor = atof(ptr) + GlobalVars::m0tor_offst; //Convert string of motor torque input to float for sprintf construction

		ptr = strtok(NULL,delim); //Iterate to next section of message

		GlobalVars::m1tor = atof(ptr) + GlobalVars::m1tor_offst; //Save second space-separated part of string to double of torque value for processing
